...
export class ChangeMenu {
  async setChangeMenu(req, body): Promise<object> {
    try {
      ...
      let role_key = await utils.judgeUserName(
        `SELECT role_key FROM router_menu WHERE  id = '${body.id}' `,
        'role_key',
        body.roleKey,
      );
      if (role_key == 1) {
      ...
      let name = await utils.judgeUserName(
        `SELECT name FROM router_menu WHERE  id = '${body.id}' `,
        'name',
        body.name,
      );
      if (name == 1) {
        let resultName = (await AppDataSource.query(
